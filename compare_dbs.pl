...
my @objs_list = qw( tables columns indexes constraints triggers );
...
if ( $number > 0 ) {
    print {$fh} "1..$number\n";
    foreach my $err ( @{$res} ) {
        print {$fh} "$counter not ok $err\n";
        $counter++;
    }
}
else {
    print {$fh} "1..1\n1 ok Schemes are equal\n";
}
close($fh);
exit $exit;
...
sub print_diff {
    ...
    foreach my $key ( sort( keys( %{$obj1} ) ) ) {
        ...
        foreach my $c_name ( sort( keys( %{ $obj1->{$key} } ) ) ) {
            unless ( $obj1->{$key}->{$c_name} eq $obj2->{$key}->{$c_name} ) {
                push( @{$result}, "$object_name elements: $c_name are not equal: "
